...
import { vehiclesApi, personnelApi, checklistsApi, complianceApi, settingsApi } from '@/lib/api';
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const sRes = await settingsApi.getAll();
        const threshold = sRes.data.alertThreshold ? parseInt(sRes.data.alertThreshold) : 30;
        
        if (sRes.data.projectName) {
          setConfig({
            projectName: sRes.data.projectName,
        ...
        const [v, p, c, certs, docs] = await Promise.all([
          ...
          complianceApi.getExpiringCertificates(threshold),
          complianceApi.getExpiringVehicleDocuments(threshold)
        ]);
        
        const now = new Date();
        
        const activeVehiclesCount = v.data.filter((veh: any) => {
          if (!veh.active) return false;
          if (veh.deactivationDate && new Date(veh.deactivationDate) <= now) return false;
          return true;
        }).length;

        const activePersonnelCount = p.data.filter((per: any) => {
          if (!per.active) return false;
          if (per.validTo && new Date(per.validTo) <= now) return false;
          return true;
        }).length;
        ...
        setStats({
          ...
          personnel: activePersonnelCount,
          totalPersonnel: p.data.length,
  ...
  const statCards = [
    { label: 'Aktif Vinçler', value: `${stats.vehicles} / ${stats.totalVehicles}`, icon: Truck, color: 'text-blue-600' },
    { label: 'Saha Personeli', value: `${stats.personnel} / ${stats.totalPersonnel}`, icon: Users, color: 'text-indigo-600' },
  ...
  return (
    <div className="space-y-6 sm:space-y-10 animate-fade pb-10">
        ...
        {loading ? (
          ...
          statCards.map((stat: any) => (
            <div key={stat.label} className="card-premium flex items-center gap-4 sm:gap-5 p-5">
              <div className={`p-3 sm:p-4 rounded-xl flex-shrink-0 bg-secondary ${stat.color}`}>
                <stat.icon size={24} />
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1 leading-none">{stat.label}</p>
                <h3 className="text-xl sm:text-2xl font-black text-foreground leading-none">{stat.value}</h3>
              </div>
            </div>
          ))
        )}
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          <Link 
            ...
            className="flex items-center justify-between p-5 bg-accent text-accent-foreground rounded-2xl hover:brightness-95 transition-all group shadow-xl shadow-accent/20 active:scale-[0.98]"
          >
      ...
      <section className="space-y-6">
        ...
        <div className="card-premium border-l-4 border-l-red-500 p-0 overflow-hidden shadow-xl">
              ...
              <div className="divide-y divide-border">
                {expiringDocs.map((doc, idx) => (
                  <div key={idx} className="py-4 flex items-center justify-between group first:pt-0 last:pb-0">
                    <div className="flex items-center gap-4">
                      <div className={`p-2 rounded-lg flex-shrink-0 ${doc.category === 'PERSONNEL' ? 'bg-primary/5 text-primary' : 'bg-secondary text-foreground'}`}>
                        {doc.category === 'PERSONNEL' ? <Users size={18} /> : <Truck size={18} />}
                      </div>
                      <div className="min-w-0">
                        <p className="font-black text-foreground text-sm sm:text-base leading-tight truncate max-w-[150px] sm:max-w-none">{doc.type}</p>
                    ...
                    <div className="text-right flex items-center gap-4 flex-shrink-0">
                      ...
                      <Link 
                        ...
                        className="p-3 text-muted-foreground hover:text-foreground transition-colors hover:bg-secondary rounded-xl border border-transparent hover:border-border"
                      >
